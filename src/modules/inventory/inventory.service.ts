...
import {
  InventoryQueryDto,
  PaginatedInventoriesResponseDto,
...
export class InventoryService {
  ...
  async getAllInventories(
    ...
    if (medicineName) {
      queryBuilder.andWhere('LOWER(medicine.name) LIKE LOWER(:medicineName)', {
        medicineName: `%${medicineName}%`,
      });
    }

    if (medicineCode) {
      queryBuilder.andWhere('LOWER(medicine.code) LIKE LOWER(:medicineCode)', {
